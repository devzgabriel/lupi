...
// Types: Actions
// biome-ignore lint/suspicious/noExplicitAny: this is a placeholder
type ExternalActionParams = any[];
type InternalActionFn<T> = (
	state: T,
	...args: ExternalActionParams
) => Promise<T> | T;
...
class Store<T, A extends ActionsDeclaration<T>> {
	...
	constructor(
		initialState: T,
		{ storage, storageKey, actions }: StoreConstructorProps<T, A>,
	) {
		...
		if (store.actions) {
			for (const actionName in store.actions) {
				externalActions[actionName] = (
					...args: Parameters<A[typeof actionName]> extends [T, ...infer P]
						? P
						: never
				) => {
